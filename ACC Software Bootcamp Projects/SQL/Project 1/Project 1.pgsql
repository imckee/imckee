...
FROM 
    departments JOIN locations ON departments.location_id = locations.location_id 
                RIGHT JOIN employees ON employees.department_id = departments.department_id
WHERE city = 'Seattle';

-- Task 5:

SELECT first_name as "FIRST", last_name as "LAST", email as "EMAIL", department_name as "DEPT", city as "CITY"
FROM 
    departments JOIN locations ON departments.location_id = locations.location_id 
                RIGHT JOIN employees ON employees.department_id = departments.department_id
WHERE city = 'Seattle';
